...
def loadCam(args, 
    ...
    # print(cam_info.image.size, resolution, id)
...
def generate_ellipse_path_from_poses(poses: np.ndarray,
    ...
    # print(offset)
    # print(np.mean(np.abs(poses[:, :3, 3]), axis=0))
    # exit()
    ...
    # # Calculate scaling for ellipse axes based on input camera positions.
    # sc = np.percentile(np.abs(poses[:, :3, 3] - offset), 10, axis=0)
    # # Use ellipse that is symmetric about the focal point in xy.
    # low = -sc + offset
    # high = sc + offset

    # positions1 = get_positions(theta)
    # print('theta[0]', theta[0])

    # if const_speed:
    #     # Resample theta angles so that the velocity is closer to constant.
    #     lengths = np.linalg.norm(positions1[1:] - positions1[:-1], axis=-1)
    #     theta = sample(None, theta, np.log(lengths), n_frames + 1)
    #     positions1 = get_positions(theta)
    ...
    # return np.stack([viewmatrix((p - center) * [1, 1, 0] + [0, 0, 0.1], up, p) for p in positions])
    # return np.stack([viewmatrix((p - center) * [1, 1, 10] , up, p1) for p, p1 in zip(positions, positions1)])
    return np.stack([viewmatrix((p - center) , up, p1) for p, p1 in zip(positions, positions)])
...
def generate_ellipse_path_from_camera_infos(
    ...
    for uid, pose in enumerate(render_poses):
        ...
        cam_info = CameraInfo(
            ...
            distance = np.linalg.norm(c2w[:3, 3] - path_center),
        )
        # print(cam_info.distance)
