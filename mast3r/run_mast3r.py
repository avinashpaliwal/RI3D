...
from colmap.read_write_model import (
    ...
    read_cameras_binary as read_intrinsics_binary,
    read_images_binary as read_extrinsics_binary,
    read_cameras_text as read_intrinsics_text,
    read_images_text as read_extrinsics_text,
...
if __name__ == "__main__":
    ...
    if not use_subdir:
        ...
        if use_calibrated_poses:
            ...
            if os.path.exists(f'{scene_path}/sparse/0'):
                ...
                src_intrinsics = {}
                src_extrinsics = {}
                valid_imgs = np.sort([f for f in os.listdir(f'{scene_path}/images') if f.endswith(('.jpg', '.png', '.jpeg', '.JPG', '.PNG', '.JPEG'))])
                for k, img_name in enumerate(valid_imgs):
    ...
    # Load images
    print('\nLoading images...')
    valid_exts = ('.jpg', '.png', '.jpeg', '.JPG', '.PNG', '.JPEG', '.bmp', '.BMP', '.webp', '.WEBP')
    all_image_names = np.sort([f for f in os.listdir(image_dir) if f.endswith(valid_exts) and not os.path.isdir(os.path.join(image_dir, f))])
    ...
    if args.image_idx is not None:
        ...
        indices = np.round(np.linspace(0, len(all_image_names) - 1, min(n_images, len(all_image_names)))).astype(int)
        image_names = [all_image_names[idx] for idx in indices]
